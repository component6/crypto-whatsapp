...
class StreamDecryptDecorator implements StreamInterface
{
    ...
    public function read($length): string
    {
        ...
        if ($this->readPos >= strlen($this->decryptedData)) {
            return '';
        }

        $res = substr($this->decryptedData, $this->readPos, $length);
    ...
    public function decrypt(): void
    {
        ...
        $this->decryptedData = $mediaCrypto->decrypt();
    }

    /**
     * Дешифрование потока по частям.
     *
     * @return string
     */
    public function decryptChunks(): string
    {
        $mediaCrypto = new MediaCrypto($this->source, $this->mediaKey, $this->mediaType);

        $this->decryptedData = $mediaCrypto->decryptChunks();
